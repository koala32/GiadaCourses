...
# Rimuovi node_modules (verrà ricreato da npm install)
rm -rf "$APP_DIR/node_modules" 2>/dev/null || true

# ── Copia i file presenti nel repo (lista esatta) ──
...
cp "$TEMP_DIR/repo/download"        "$APP_DIR/" 2>/dev/null || true
...
[Unit]
Description=GiadaCourses v8.0 - Social English Learning Platform (Security Hardened)
...
echo -e "  ${CYAN}tail -n 1 /opt/giadacourses/index.html${NC}    # deve finire con </html>"
...
echo -e "  ${CYAN}sudo bash /opt/giadacourses/DEPLOY.sh${NC}     # ri-deploy"
echo -e "  ${CYAN}sudo bash /opt/giadacourses/BACKUP.sh${NC}     # backup manuale"
echo -e "  ${CYAN}sudo bash /opt/giadacourses/ROLLBACK.sh${NC}   # rollback emergenza"
echo ""
echo -e "${BOLD}${GREEN}══════════════════════════════════════════════════${NC}"
