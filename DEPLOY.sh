...
server {
    ...
    # ── SECURITY: blocca scanner e exploit paths ──
    location ~* /(vendor|\.env|\.git|wp-admin|wp-login|wp-content|xmlrpc|phpmyadmin|cgi-bin|\.aws|\.config|actuator|telescope|elfinder) {
        access_log off;
        return 403;
    }
    ...
    location /uploads/ {
        ...
        # Blocca completamente accesso diretto ai file APK
        location ~* \.apk$ { return 403; }
        try_files $uri =404;
    }
