  ...
  modal.innerHTML=`
    ...
    <div style="width:100%;max-width:500px;padding:16px 16px calc(20px + var(--sab,0px))">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px">
        ...
        <button class="sc-action-btn sc-secondary" onclick="document.getElementById('story-tutorial')?.remove();localStorage.setItem('gc_story_tutorial','1');openStoryCreator();"><span>&#x2753;</span> Tutorial</button>
      </div>
...
async function startLive(){
  ...
  if(!ME||!(ME.username?.toLowerCase()==='giada'||ME.role==='superadmin')){toast('Solo Giada può fare dirette','error');return;}
  ...
  try{
    ...
    catch{stream=await navigator.mediaDevices.getUserMedia({audio:true,video:false});toast('📷 Camera non disponibile, solo audio','info',2000);}
    ...
    vid.muted=true; // Host vede se stesso ma non si sente (feedback loop)
    document.getElementById('live-title-lbl').textContent=title;
    document.getElementById('live-host-controls').style.display='flex';
    document.getElementById('live-overlay').classList.add('active');
    document.getElementById('live-comments-scroll').innerHTML='<div style="color:rgba(255,255,255,.4);font-size:.78rem;padding:4px 0">💬 I commenti appariranno qui...</div>';
    toast('🔴 Diretta avviata! Gli spettatori vedranno la tua diretta live.','success',4000);
    console.log('[LIVE] Host started stream', streamId);
  }catch(e){toast('Errore avvio diretta: '+e.message,'error');}
}
...
function startSSE(){
  ...
  onEvent('live_viewer_joined', async d => {
    ...
    async function createOfferForViewer(viewerId) {
      ...
      pc.onicecandidate = ev => {
        if (ev.candidate) POST('/api/live/ice/'+d.streamId, {candidate:ev.candidate, targetUserId:viewerId}).catch(()=>{});
      };
      pc.onconnectionstatechange = () => {
...
// ============================================================
//  FIX LIVE: admin can also start live (not just Giada)
// ============================================================
...
window.startLive=async function(){
  ...
  try{
    ...
    document.getElementById('live-overlay')?.classList.add('active');
    const cs=document.getElementById('live-comments-scroll');
