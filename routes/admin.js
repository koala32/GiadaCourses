// ═══════════════════════════════════════════════════════════
//  Admin Routes — Dashboard superadmin
// ═══════════════════════════════════════════════════════════
...
router.get('/stats', (req, res) => {
  ...
  res.json({
    totalUsers, activeToday, openTickets, totalTickets, resolvedTickets, bannedUsers,
    recentRegistrations, ticketsByCategory
  });
});
...
router.get('/users', (req, res) => {
  ...
  // Aggiungi stats per ogni utente
...
router.put('/users/:id/ban', requireSuperAdmin, (req, res) => {
  ...
  db.prepare('UPDATE users SET is_banned = 1, ban_reason = ?, updated_at = datetime("now") WHERE id = ?')
    .run(reason || 'Violazione regolamento', req.params.id);
  logActivity(db, req.user.id, 'ban_user', `Bannato utente ${req.params.id}: ${reason}`, req.ip);
...
router.put('/users/:id/unban', requireSuperAdmin, (req, res) => {
  ...
  logActivity(db, req.user.id, 'unban_user', `Sbannato utente ${req.params.id}`, req.ip);
  res.json({ success: true });
});
...
router.get('/tickets', (req, res) => {
  ...
  let query = `
    SELECT t.*, u.display_name, u.username, u.email,
  ...
  const total = db.prepare(`SELECT COUNT(*) as n FROM tickets ${status && status !== 'tutti' ? 'WHERE status = ?' : ''}`).get(...(status && status !== 'tutti' ? [status] : [])).n;
...
router.put('/tickets/:id/status', (req, res) => {
  ...
  const extra = status === 'chiuso' ? `, closed_at = datetime("now"), closed_by = "${req.user.id}"` : '';
