// ═══════════════════════════════════════════════════════════
//  Learn Routes — Corsi, lezioni, quiz, progresso
// ═══════════════════════════════════════════════════════════
...
router.get('/courses', optionalAuth, (req, res) => {
  ...
  // Se autenticato, aggiungi progresso
  if (req.user) {
    const enriched = courses.map(c => {
      const totalLessons = c.lesson_count;
      ...
      return { ...c, completedLessons: completed, progress: totalLessons > 0 ? Math.round((completed / totalLessons) * 100) : 0 };
    });
...
router.get('/courses/:id', optionalAuth, (req, res) => {
  ...
  if (req.user) {
    ...
    const enrichedLessons = lessons.map(l => ({ ...l, progress: progressMap[l.id] || null }));
    return res.json({ ...course, lessons: enrichedLessons });
  }

  res.json({ ...course, lessons });
});

// ── Contenuto lezione (richiede auth) ───────────────────────
...
router.post('/lessons/:id/complete', authenticateToken, (req, res) => {
  ...
  db.prepare(`
    ...
    ON CONFLICT(user_id, lesson_id) DO UPDATE SET completed = 1, score = ?, completed_at = datetime('now')
  `).run(req.user.id, req.params.id, score || null, score || null);

  res.json({ success: true });
});

// ── Progresso globale utente ────────────────────────────────
router.get('/progress', authenticateToken, (req, res) => {
  const progress = db.prepare(`
    SELECT up.*, l.title as lesson_title, l.type, c.title as course_title, c.icon as course_icon
